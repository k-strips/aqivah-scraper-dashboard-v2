...
async function fetchProperty({ showLoader, hideLoader, id, notify, setProperty }) {
  try {
    showLoader();
    ...
    <Card>
      <Card.Body>
        <Row style={{ marginBottom: '30px', }}>
          <Col>
            <h4>URL</h4>
            <a href={property?.url}><h5>{property?.url}</h5></a>
          </Col>
          <Col>
            <h4>Created At</h4>
        ...
        <Row>
          {property.PropertyDetails?.map(each => {
            const {
              SourceField: { Field: { label: title = '' }, }, details: value,
            } = each || {};

            return <Col key={each?.id}>
