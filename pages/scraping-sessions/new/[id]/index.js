...
async function fetchScrapingSession({
  showLoader,
  hideLoader,
  id,
  notify,
  ...
  try {
    ...
    const formattedProperties = response?.Properties?.reduce(
      (final, each) => {
        return {
    ...
    actions: {
      ...
      getValue: (record) => (
        <FieldActions
          onClickView={(router) =>
            router.push(
              `/scraping-sessions/new/${router.query.id}/properties/${record?.id}`
            )
          }
        />
      ),
...
function ScrapingSessionsView() {
  ...
  return (
    <DashboardLayout heading={`Scraping Sessions > New > ${id}`}>
